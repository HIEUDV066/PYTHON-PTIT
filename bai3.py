...
class Triangle:
  ...
  def solve(self):
    xy=self.distance(self.x1,self.y1,self.x2,self.y2)
    yz=self.distance(self.x2,self.y2,self.x3,self.y3)
    xz=self.distance(self.x3,self.y3,self.x1,self.y1)
    r = sqrt((xy+yz+xz)*(xy+yz-xz)*(yz+xz-xy)*(xz+xy-yz))
    print("%.2f" % (r*0.25))
